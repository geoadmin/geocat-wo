...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Google Analytics 4 (gtag.js) - Load in head -->
  ...
  <!-- Google Analytics 4 (gtag.js) - Fallback for noscript -->
  <xsl:template name="ga4-load-body">
    <xsl:choose>
      <xsl:when test="$isProd">
        <noscript>
          <iframe src="https://www.googletagmanager.com/ns.html?id=G-C0NWPQVB2N" height="0" width="0" style="display:none;visibility:hidden"></iframe>
        </noscript>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
